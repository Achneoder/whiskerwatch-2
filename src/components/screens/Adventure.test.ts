...
describe('Adventure', () => {
  ...
  it('adds a sub-beat under an existing beat', async () => {
    replaceBeats([{ id: 'root', parentId: null, title: 'Root beat', notes: '', status: 'planned' }]);
    render(Adventure, { props: { onnavigate: vi.fn() } });
  ...
  it('deletes a beat and its sub-beats after confirming, warning about the count', async () => {
    replaceBeats([
      { id: 'root', parentId: null, title: 'Root beat', notes: '', status: 'planned' },
      { id: 'child', parentId: 'root', title: 'Child beat', notes: '', status: 'planned' },
    ]);
    render(Adventure, { props: { onnavigate: vi.fn() } });
